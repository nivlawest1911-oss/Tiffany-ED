...
const project = process.env.GCP_PROJECT_ID || 'edintel-sovereign';
...
export async function generateGrantDraft(prompt: string) {
    try {
        ...
        const request = {
            contents: [{ role: 'user', parts: [{ text: `You are a professional educational grant writer for the Mobile County school district. Respond as a "Sovereign OS Intelligence". Prompt: ${prompt}` }] }],
        };
