...
import { Activity, ShieldAlert, Users, Brain, BarChart3, Lock, TrendingUp } from 'lucide-react';
    ...
    const COMMAND_NODES = [
        {
            ...
            actions: [
                ...
                { label: 'Sovereign ROI', href: '/admin/roi', icon: <TrendingUp className="w-4 h-4" /> },
                { label: 'Executive Roleplay', href: '/admin/roleplay', icon: <Brain className="w-4 h-4" /> }
            ]
        },
        {
            ...
            actions: [
                { label: 'Roster Logic', href: '/roster', icon: <Users className="w-4 h-4" /> },
                { label: 'Restorative Coach', href: '/admin/restorative', icon: <ShieldAlert className="w-4 h-4" /> },
                { label: 'Suspension Alt', href: '/admin/alternatives', icon: <ShieldAlert className="w-4 h-4" /> }
            ]
        },
        {
        ...
        <div className="max-w-7xl mx-auto space-y-12 pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {COMMAND_NODES.map((node, i) => (
                    <SmartHover key={node.title} message={node.description}>
                        <motion.div
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ delay: i * 0.1 }}
                            className="p-8 rounded-[2.5rem] bg-black/40 border border-white/5 backdrop-blur-3xl relative overflow-hidden group hover:border-intel-gold/30 transition-all h-full"
                        >
