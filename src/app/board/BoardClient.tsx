...
function PanelSkeleton({ h = 280 }: { h?: number }) {
  return (
    <div
      className="w-full animate-pulse rounded-2xl bg-zinc-200/60 dark:bg-zinc-800/60"
      style={{ height: h }}
      aria-hidden
    />
  );
...
// Recharts + data panels — separate chunks, only when this route mounts
const AuditChart = dynamic(() => import('@/components/AuditChart'), {
  ssr: false,
  loading: () => <PanelSkeleton h={280} />,
});
const ResourceMap = dynamic(() => import('@/components/ResourceMap'), {
  ssr: false,
  loading: () => <PanelSkeleton h={320} />,
});
const BoardReport = dynamic(() => import('@/components/BoardReport'), {
  ssr: false,
  loading: () => <PanelSkeleton h={240} />,
});
const RevenueDashboard = dynamic(() => import('@/components/RevenueDashboard'), {
  ssr: false,
  loading: () => <PanelSkeleton h={240} />,
});
const GovernanceEngine = dynamic(() => import('@/components/GovernanceEngine'), {
  ssr: false,
  loading: () => <PanelSkeleton h={200} />,
});
const BurnoutHeatmap = dynamic(() => import('@/components/BurnoutHeatmap'), {
  ssr: false,
  loading: () => <PanelSkeleton h={260} />,
});
      ...
      <div className="max-w-6xl mx-auto space-y-12">
        <motion.header variants={item} className="text-center space-y-4 relative pt-6">
          ...
          <p className="text-lg text-zinc-500 dark:text-zinc-400 font-medium">
            Mobile County District Data Visualization - Project Alpha
          </p>
        </motion.header>
        ...
        <motion.footer
          variants={item}
          className="pt-12 mt-12 border-t border-zinc-200 dark:border-zinc-800 text-center"
        >
          ...
          <Link
            href="/login"
            className="inline-block mt-4 text-xs font-bold text-zinc-300 hover:text-blue-500 transition-colors uppercase tracking-widest"
          >
