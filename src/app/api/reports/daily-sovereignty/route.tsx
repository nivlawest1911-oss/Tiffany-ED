...
import DailySovereignPDF from '@/components/reports/DailySovereignPDF';
...
export async function GET(request: Request) {
    ...
    if (!isCron && !isDev && !isAuthorizedTest) {
        return new Response('Unauthorized - Sovereign clearance required.', { status: 401 });
    }
    ...
    try {
        ...
        const pdfBuffer = await renderToBuffer(<DailySovereignPDF data={data} />);
        ...
        const result = await resend.emails.send({
            ...
            subject: `Daily Sovereignty Report | ${data.district} | ${new Date().toLocaleDateString()}`,
            attachments: [
                {
                    filename: `Sovereign_Report_${data.district.replace(/\s+/g, '_')}.pdf`,
                    content: pdfBuffer,
                },
            ],
            text: `Your daily leadership and neuro-resilience metrics for ${data.district} are now available. View the attached Sovereignty Report.`,
        });
        ...
        try {
            ...
            await resend.emails.send({
                ...
                subject: 'ALERT: Daily Sovereignty Report Transmission Failure',
                text: `
                    Attention EdIntel Command Center,

                    The Daily Sovereignty Report for Mobile County failed to transmit.

                    Error Details: ${error.message}

                    Potential Infrastructure Blocks Identified:
                    1. Google Cloud Project 'edintel-sovereign-2026' may be suspended.
        ...
        return Response.json({ error: 'Sovereignty Report failed to synthesize.', details: error.message }, { status: 500 });
    }
}
