...
export default function AidePanel({ isOpen, mode, onClose }: AidePanelProps) {
    ...
    return (
        <div className={`fixed inset-y-0 right-0 w-96 bg-white dark:bg-zinc-900 shadow-2xl z-[100] transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-full'} border-l border-zinc-200 dark:border-zinc-800 flex flex-col`}>
            {/* Header */}
            <div className="p-4 border-b dark:border-zinc-800 flex justify-between items-center bg-zinc-50 dark:bg-zinc-900/50">
                ...
                <button onClick={onClose} className="p-1 hover:bg-zinc-200 dark:hover:bg-zinc-800 rounded-full" aria-label="Close panel"><X size={20} /></button>
            </div>
            ...
            <div className="p-4 border-t dark:border-zinc-800">
                <div className="relative">
                    ...
                    />
                    <button onClick={handleSendMessage} className="absolute right-2 top-2 p-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700" aria-label="Send message">
                        <Send size={16} />
                    </button>
                </div>
            </div>
