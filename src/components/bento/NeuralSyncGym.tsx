            ...
            <div className="relative z-10">
                <div className="flex items-center justify-between mb-10">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            <h2 className="text-3xl font-black tracking-tighter">Neural Sync Gym</h2>
                            <p className="text-sm font-bold text-cyan-400 uppercase tracking-widest">Cognitive Fitness Training x10</p>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-4 gap-3 mb-10">
                    {[
                        { id: 'memory', label: 'Working Memory', icon: <Cpu size={16} /> },
                        { id: 'inhibition', label: 'Pulse Control', icon: <ZapOff size={16} /> },
                        { id: 'speed', label: 'Math Flux', icon: <Zap size={16} /> },
                        { id: 'logic', label: 'Pattern Match', icon: <Eye size={16} /> }
                    ].map((m) => (
                        <button
                            key={m.id}
                    ...
                    {!gameActive ? (
                        <div className="text-center space-y-6">
                            <h3 className="text-xl font-bold text-zinc-300">
                                ...
                                {mode === 'inhibition' && "Color Interference Lab"}
                                {mode === 'speed' && "Arithmetic Velocity"}
                                {mode === 'logic' && "Visual Matrix Search"}
                            </h3>
                            <p className="text-sm text-zinc-500 max-w-sm mx-auto">
                                {mode === 'memory' && "Expand your working memory buffer by recalling increasing sequences."}
                                {mode === 'inhibition' && "Click the NAME of the color, ignoring the visual text color. Trains inhibitory control."}
                                {mode === 'speed' && "Solve rapid arithmetic equations to increase neuroplasticity and processing speed."}
                                {mode === 'logic' && "Identify the missing piece of the pattern. Improves abstract reasoning."}
                            </p>
                            <button
                                onClick={startGame}
                                className="px-10 py-5 bg-gradient-to-r from-cyan-600 to-blue-600 rounded-2xl font-black text-sm uppercase tracking-[0.2em] shadow-2xl shadow-cyan-900/40 hover:scale-105 transition-all flex items-center gap-3 mx-auto"
                            >
                                <RefreshCw size={18} />
                                Calibrate Neural Path
                            </button>
                        </div>
                    ) : (
                ...
                <div className="mt-12">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        <TrainingNode title="Student Focus" description="Executive function & recall" color="cyan" icon={<Brain size={14} />} />
                        <TrainingNode title="Teacher Guard" description="Burnout shield & focus" color="blue" icon={<ShieldCheck size={14} />} />
                        <TrainingNode title="Admin Vision" description="Strategic decision velocity" color="purple" icon={<Target size={14} />} />
                        <TrainingNode title="Parent Sync" description="Cognitive patience & calm" color="orange" icon={<Heart size={14} />} />
                    </div>
                </div>
            </div>
        </div>
...
function TrainingNode({ title, description, color, icon }: { title: string, description: string, color: string, icon: React.ReactNode }) {
