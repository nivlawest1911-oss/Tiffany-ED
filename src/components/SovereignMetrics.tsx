...
export default function SovereignMetrics() {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setStats(prev => ({
                latticeSync: Math.min(100, prev.latticeSync + (Math.random() * 0.1 - 0.05)),
                defenseRating: prev.defenseRating,
                strategicAgility: Math.min(100, prev.strategicAgility + (Math.random() * 0.2 - 0.1)),
    ...
    const metricItems = [
        { label: 'Lattice Sync', value: stats.latticeSync, icon: Activity, color: 'text-indigo-400', bg: 'bg-indigo-500/10' },
        { label: 'Defense Rating', value: stats.defenseRating, icon: Shield, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
        { label: 'Strategic Agility', value: stats.strategicAgility, icon: Zap, color: 'text-amber-400', bg: 'bg-amber-500/10' },
        { label: 'Distict Impact', value: stats.impactScore, icon: Target, color: 'text-rose-400', bg: 'bg-rose-500/10' },
    ];
