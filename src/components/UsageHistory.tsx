        ...
        <div className="bg-[#050507] border border-white/5 rounded-[2rem] overflow-hidden backdrop-blur-md shadow-2xl group">
            <div className="p-6 border-b border-white/5 bg-gradient-to-r from-indigo-500/5 to-transparent flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-[10px] tracking-[0.3em] uppercase text-zinc-300 font-black">EdIntel Ledger</h3>
                        <p className="text-[8px] text-zinc-400 uppercase font-bold">Encrypted Transaction History</p>
                    </div>
                </div>
                <button className="p-2 rounded-lg bg-white/5 border border-white/10 hover:border-indigo-500/30 transition-all group/btn" aria-label="Download ledger history">
                    <Download className="w-3 h-3 text-zinc-400 group-hover/btn:text-indigo-400 transition-colors" aria-hidden="true" />
                </button>
            </div>
            ...
            <div className="divide-y divide-white/5 max-h-[300px] overflow-y-auto custom-scrollbar">
                {logs && logs.length > 0 ? logs.map((log, i) => (
                    ...
                    >
                        <div className="flex items-center gap-4">
                            <div className={`w-9 h-9 rounded-xl flex items-center justify-center border transition-colors ${log.amount > 0
                                ...
                                {log.amount > 0 ? <ArrowDownLeft size={16} aria-hidden="true" /> : <ArrowUpRight size={16} aria-hidden="true" />}
                            </div>
                            <div>
                                ...
                                <p className="text-[8px] text-zinc-400 uppercase font-mono mt-1">
                                    {new Date(log.created_at).toLocaleString()} {" // "} REF: {log.id.substring(0, 8)}
                                </p>
                            </div>
                        </div>
                        <div className="text-right">
                            <span className={`text-xs font-black font-mono ${log.amount > 0 ? 'text-emerald-400' : 'text-zinc-300'}`}>
                                {log.amount > 0 ? `+${log.amount}` : log.amount} U
                            </span>
                            <p className="text-[7px] text-zinc-400 uppercase font-bold tracking-widest mt-1">
                                Verified
                            </p>
                        </div>
                    </motion.div>
                    ...
                    <div className="p-12 text-center">
                        <div className="text-[10px] text-zinc-400 uppercase font-black tracking-widest">No Transactions Detected</div>
                        <div className="text-[8px] text-zinc-500 uppercase mt-2">Ledger is active and awaiting input</div>
                    </div>
                )}
            </div>

            <div className="p-4 bg-white/[0.01] border-t border-white/5 text-center">
                <span className="text-[7px] text-zinc-400 uppercase font-black tracking-[0.4em]">District Audit Readiness: High</span>
            </div>
        </div>
    );
};
