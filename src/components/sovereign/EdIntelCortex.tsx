...
const NEURAL_NODES: NeuralNode[] = [
    // Intelligence Tier
    { id: 'vertex', name: 'Vertex AI EdIntel Core', icon: Cpu, status: 'active', category: 'Intelligence', load: 92 },
    ...
    { id: 'storage', name: 'EdIntel Media Storage', icon: HardDrive, status: 'synced', category: 'Data', load: 58 },
...
export default function EdIntelCortex() {
    ...
    return (
        <div className="w-full bg-[#050507] border border-white/5 rounded-[3rem] overflow-hidden shadow-2xl">
            {/* Header */}
            <div className="p-8 border-b border-white/5 bg-gradient-to-br from-noble-gold/5 to-transparent">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        ...
                        <h2 className="text-3xl font-black text-white tracking-tighter uppercase leading-none">
                            EdIntel <span className="gold-gradient-text">Cortex</span>
                        </h2>
                        ...
                        {['Intelligence', 'Data', 'Infrastructure', 'Outreach', 'Media'].map(cat => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${selectedCategory === cat
                                    ? 'bg-noble-gold text-black border-noble-gold shadow-[0_0_15px_rgba(212,175,55,0.4)]'
                ...
                <AnimatePresence mode="popLayout">
                    {filteredNodes.map((node, i) => (
                        <motion.div
                            ...
                            className="p-5 rounded-2xl bg-white/[0.02] border border-white/5 hover:border-noble-gold/30 group transition-all relative overflow-hidden"
                        >
                            {/* Animated Background Pulse */}
                            {node.status === 'active' && (
                                <motion.div
                                    className="absolute inset-0 bg-noble-gold/5"
                            ...
                            <div className="flex items-start justify-between mb-4 relative z-10">
                                <div className={`w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center border border-white/5 transition-colors group-hover:bg-noble-gold/10 group-hover:border-noble-gold/20`}>
                                    <node.icon className={`w-5 h-5 ${node.status === 'active' ? 'text-noble-gold' :
                                        node.status === 'synced' ? 'text-emerald-400' :
                                            'text-zinc-600'
                                        }`} />
                                </div>
                                <div className={`px-2 py-0.5 rounded-full text-[7px] font-black uppercase tracking-widest border ${node.status === 'active' ? 'bg-noble-gold/10 text-noble-gold border-noble-gold/20' :
                            ...
                            <div className="relative z-10">
                                <h4 className="text-[10px] font-black text-white uppercase tracking-wider mb-3 leading-tight group-hover:text-noble-gold transition-colors">
                                    {node.name}
                                </h4>
                                ...
                                <div className="space-y-2">
                                    <div className="flex justify-between items-center text-[8px] font-bold text-zinc-500 uppercase tracking-tighter">
                                        ...
                                        <span className={node.load > 80 ? 'text-emerald-400' : 'text-noble-gold'}>{Math.round(node.load)}%</span>
                                    </div>
                                    <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                                        <motion.div
                                            className={`h-full ${node.load > 80 ? 'bg-emerald-500 shadow-[0_0_8px_#10b981]' : 'bg-noble-gold shadow-[0_0_8px_#D4AF37]'}`}
            ...
            <div className="px-8 py-6 bg-white/[0.02] border-t border-white/5 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="flex items-center gap-6">
                    ...
                    <div className="flex items-center gap-2">
                        <Globe className="w-3 h-3 text-noble-gold" />
                ...
                <div className="text-[9px] font-mono text-zinc-600 uppercase tracking-tighter">
                    Mainframe: edintel-EdIntel-2027 // ARCHITECTURE: v4.2-2027-EdIntel
                </div>
            </div>
        </div>
    );
