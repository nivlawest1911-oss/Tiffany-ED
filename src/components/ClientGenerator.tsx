...
export default function ClientGenerator({ generatorId }: { generatorId: string }) {
    ...
    if (!generator) return <div className="min-h-screen bg-[#050a14] flex items-center justify-center text-white"><Loader2 className="animate-spin mr-2" /> Initializing Sovereign Node...</div>

    // --- LOGIC REUSED FROM HUB ---
    const getFallbackResponse = (genId: string, userPrompt: string) => {
        return `### Sovereign Intelligence: ${generator.name} Response
...
*Generated by EdIntel Local Neural Node v4.0*`
    ...
    const handleVoiceInput = () => {
        if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
            ...
            alert("Voice Command Node not supported in this browser sector. Please use Chrome.")
        }
    }
        ...
        <div className="min-h-screen bg-[#050a14] text-white">
            ...
            <main className="max-w-7xl mx-auto px-4 py-8 space-y-24">
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        <div className="glass-card p-6 rounded-3xl relative overflow-hidden group">
                            ...
                            <div className="flex flex-wrap gap-2">
                                ...
                                <span className="px-3 py-1 bg-white/5 rounded-full text-xs border border-white/10">Neural Engine</span>
                            </div>
                        </div>
                ...
                <section id="about-delegate" className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center scroll-mt-32 border-t border-white/5 pt-16">
                    <div>
                        ...
                        <p className="text-lg text-gray-300 leading-relaxed mb-8">
                            This Sovereign Agent is not a generic model. It is a specialized neural node trained on district policy, cognitive science, and Alabama state code.
                ...
                <section id="related" className="scroll-mt-32 border-t border-white/5 pt-16">
                    <h2 className="text-3xl font-black text-white mb-8">Related Sovereign Nodes</h2>
                        ...
                        {generators.filter(g => g.id !== generator.id).slice(0, 3).map(related => (
                            <Link key={related.id} href={`/generators?id=${related.id}`} className="group p-6 glass-card rounded-2xl hover:border-[#00d2ff]/50 transition-all">
                                ...
                                <div className="flex items-center gap-2 text-xs font-bold text-[#00d2ff] uppercase tracking-wider">
                                    Initialize Node <ExternalLink className="w-3 h-3" />
                                </div>
                            </Link>
                        ))}
                    </div>
            ...
            <footer className="py-12 text-center text-gray-600 text-sm">
                EdIntel Sovereign System v4.0.2 // ID: {generator.id.toUpperCase()}
            </footer>
        </div>
    )
}
