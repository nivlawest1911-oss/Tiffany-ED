...
interface EdIntelBriefingProps {
    summary: string;
    loading?: boolean;
    videoSrc?: string;
}
...
export const EdIntelBriefing = ({ summary, loading }: EdIntelBriefingProps) => {
    ...
    return (
        <HolographicCard
            ...
            subtitle="Executive Briefing Active"
            icon={<Activity className="text-primary-500 w-5 h-5" />}
            className="w-full"
        >
            <div
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-4">
                            {points.map((point, idx) => (
                                ...
                                >
                                    <div className="mt-1.5 h-1.5 w-1.5 rounded-full bg-primary-500 shadow-[0_0_8px_rgba(99,102,241,0.5)] group-hover:scale-125 transition-transform" />
                                    <div>
                                        <span className="text-[10px] font-black text-primary-400 uppercase tracking-widest block mb-0.5">
                                            {point.label}
                                        </span>
                                        <span className="text-sm text-white/80 font-medium leading-tight group-hover:text-white transition-colors">
                                            {point.value}
                                        </span>
                                    </div>
                                </motion.div>
                ...
                <AnimatePresence>
                    {isHovered && (
                        <motion.div
                            ...
                            animate={{ opacity: 1 }}
                            exit={{ opacity: 0 }}
                            className="absolute -top-12 right-0 flex gap-2 pointer-events-none"
                        >
