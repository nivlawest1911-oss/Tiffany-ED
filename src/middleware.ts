...
export async function middleware(request: NextRequest) {
    ...
    if (pathname.startsWith('/api/fleet') || pathname.startsWith('/api/wellness')) {
        try {
            ...
            // Rate limiting unavailable, allow request to proceed
            console.warn('[MIDDLEWARE] Rate limiting unavailable:', e);
        }
    }
    ...
    if (pathname.startsWith('/api/auth') || pathname.includes('login') || pathname.includes('signup')) {
        try {
            ...
            // Rate limiting unavailable, allow request to proceed
            console.warn('[MIDDLEWARE] Auth rate limiting unavailable:', e);
        }
    }
