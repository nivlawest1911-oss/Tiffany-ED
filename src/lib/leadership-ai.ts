// Professional AI Engine - Robust Local Intelligence
...
const TEMPLATES = {
  DEFAULT: (topic: string, persona: typeof USER_CREDENTIALS, isChat: boolean) => {
    if (isChat) {
      return `Hello. Dealing with ${topic || 'this situation'} requires a steady hand, doesn't it? ... Drawing from my experience as ${persona.role}, I'd suggest we focus on three things: fidelity to our mission, fiscal discipline, and cultural resonance. I can draft a detailed plan for you right now, but first... let's start with a small pilot protocol. Does that sound like a solid first step to you?`;
    }
    return `
# 🦁 Professional Executive Briefing
**Generated from the Workspace of ${persona.name}**
**Theme:** Strategic Leadership | **Timestamp:** [Live Strategic Connection]
...
## Strategic Pillars
1.  **Fidelity to Mission:** This directive strengthens our "Strategic Network" of student success.
...
*"Success is not an accident; it's a strategic imperative."*
  ...
  IEP: (topic: string, persona: typeof USER_CREDENTIALS, isChat: boolean) => {
    if (isChat) {
      return `Alright, let's take a serious look at this IEP for ${topic || 'the student'}. ... Looking at the data, I see a gap between their potential and their current performance. Honestly? We can do better than this current draft. Ideally, we want to set some SMART goals here that actually mean something, not just compliance checkboxes. I'm thinking we focus on executive mastery and maybe some digital scaffolding to help with the memory load. We need to make sure this is fully compliant with IDEA mandates too. ... Shall I draft the full goals for you now?`;
    }
    return `
# 🏛️ Comprehensive Individualized Education Program (IEP) Strategy Dossier
**Subject Center:** ${topic || 'Individualized Support Protocol'} | **Security Level:** FERPA-Alpha
**Strategic Architect:** ${persona.name}, ${persona.role}
**Timestamp:** [Strategic Sync Active]
...
## 2. SMART Goals & High-Yield Leadership Objectives
...
## 3. Professional Accommodations & Environmental Engineering
*   **Strategic Offloading:** Provide digital scaffolding for high-load memory tasks to prioritize conceptual sovereignty.
...
|:---|:---|:---|:---|
| 01 | Primary Protocol Rollout | SPED Center | 48 Hours |
...
*"We don't lower the bar; we build the ladder."*
  ...
  LESSON: (topic: string, persona: typeof USER_CREDENTIALS, isChat: boolean) => {
    if (isChat) {
      return `Okay, for a lesson on ${topic || 'this topic'}, we need to really hook them first—capture their imagination before we even open the textbook. I'd suggest starting with a 5-minute provocation that links this to something cultural or technological they care about. ... Then we move into a collaborative "Foundry" phase where they build the mental model before we even lecture. How does that flow sound to you?`;
    }
    return `
# 🎓 Strategic Instructional Protocol: Excellence-Driven Design
...
## 2. Evidence of Leadership (The Audit)
*   **Performance Prototype:** Students will not just "know"—they will "build." They will engineer a solution using **${topic || 'the content'}** to address a real-world village challenge.
...
1.  **Strategic Hook (Engage):** (5 min) A high-intensity provocation that links **${topic || 'the topic'}** to a cultural or technological trend.
2.  **The Foundry (Explore):** (15 min) Collaborative hands-on inquiry. Building the mental model before the lecture.
3.  **Professional Synthesis (Explain):** (10 min) Direct instruction with high-resonance visuals. Zero fluff, 100% data.
...
*   **Level 3 (Leadership):** Independent research uplink.
...
**District Value:** This lesson protocols shortens the learning curve by 20% compared to legacy instructional methods.
  ...
  EMAIL: (topic: string, persona: typeof USER_CREDENTIALS, isChat: boolean) => {
    if (isChat) {
      return `I can definitely help you draft that email about ${topic}. The key here is to be concise but authoritative. I'd frame it around our "Excellence Without Excuse" mission. I'll summarize our current progress, note any friction points, and then... give a clear directive for the next steps. Want me to write out the full draft now?`;
    }
    return `
...
2. Prepare your team for the "Professional Shift" on Monday.
...
**${persona.name}**
*${persona.role}*
*Professional Leadership Solutions*
  `;
  },
  FINANCE: (topic: string, persona: typeof USER_CREDENTIALS, isChat: boolean) => {
    if (isChat) {
      return `Looking at the numbers for ${topic}, my analysis as a DBA in Finance suggests the ROI here is actually quite high. ... It's an investment in intellectual leadership, plain and simple. We shouldn't dip into the General Fund if we don't have to—maybe we use some carryover or look for a matching grant? What do you think about reallocating some resources?`;
    }
    return `
# 📈 Fiscal Solvency & ROI Analysis
...
*   **Strategic ROI:** High. The long-term dividend of "Intellectual Leadership" far outweighs the initial fiscal friction.
...
The data is clear. Proceeding with this investment in ${topic || 'this area'} is the most fiscally sound path towards sustained excellence and district-wide equity.
  ...
  COMMUNITY: (topic: string, persona: typeof USER_CREDENTIALS, isChat: boolean) => {
    if (isChat) {
      return `You know, my Grandmother used to say, "if you want to go fast, go alone; if you want to go far, go together." ... Regarding ${topic}, we need to mobilize the village. I'm seeing a need for equity over equality here. Let's call on parents and elders to really step up. It's about collective excellence.`;
    }
    return `
# 🌍 The Village Communique
...
I have used my "Strategic Insights" to look at the data, and here is what it tells me: Our children are capable of 100% success if we provide 100% support for ${topic || 'their development'}.
...
3. **Step Up:** Join our "Professional Support Team" to mentor our youth.

*"We are the ones we've been waiting for. We are the architects of our own excellence."*
    `;
  },
  BEHAVIOR: (topic: string, persona: typeof USER_CREDENTIALS, isChat: boolean) => {
    if (isChat) {
      return `Behavior is communication, plain and simple. For ${topic}, we need to ask: what is the student trying to tell us? ... I'd recommend we look at antecedent engineering—basically, changing the environment to stop the trigger before it happens. We want to build their self-regulation muscles, not just force compliance.`;
    }
    return `
# 🛡️ Behavioral Engineering & Intervention Protocol
...
## 2. Professional Intervention Plan
...
This is not about compliance; it's about **Self-Regulation Leadership**. We are building the student's internal HUD for decision making.
  `;
  },
  WELLNESS: (topic: string, persona: typeof USER_CREDENTIALS, isChat: boolean) => {
    if (isChat) {
      return `We can't build excellence on burnout. I'm sensing some stress around ${topic}. ... We need a strategic reset. Maybe we use some of the EdIntel tools to offload the busy work, or implement a "Breather Protocol" for the staff. Wellness is a strategic asset, not a luxury.`;
    }
    return `
# 🧘 Strategic Wellness & Resilience Protocol
...
*   **The Breather Protocol:** Implementing 5-minute "Strategic Resets" twice daily for staff and students.
...
Wellness is a strategic asset. A regulated adult lead to a regulated child, which leads to an Elite District.
  ...
  OPERATIONS: (topic: string, persona: typeof USER_CREDENTIALS, isChat: boolean) => {
    if (isChat) {
      return `Operational excellence is the quiet engine of success. For ${topic}, I think we can optimize throughput by about 15%. ... Let's look at automating some of the repetitive tasks and ensuring we are 100% compliant with safety protocols. We need to move resources from overhead to direct student impact.`;
    }
    return `
# ⚙️ Logistical Leadership & Operational Audit
...
*   **Automation Connection:** Shifting repetitive tasks to the Professional Engine.
...
export async function generateProfessionalResponse(
  ...
  return TEMPLATES.DEFAULT(topic, activePersona, isChat);
}
